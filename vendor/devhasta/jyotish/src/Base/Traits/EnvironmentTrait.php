...
trait EnvironmentTrait
{
    ...
    {
////        $Graha = Graha::$graha;
////        $Graha = Graha::getInstance('Su', null);
//
//        $grahaClass = 'Jyotish\Graha\Object\Su';
//        $grahaObject = new $grahaClass(null);
//
////        'Su' => 'Surya',
//
//        echo '<pre>';
//        var_export($grahaObject);
//        echo '</pre>';
//
//        return true;
    ...
    public function isAspectArr($options = null)
    {
        $result = [];
        foreach (Graha::$graha as $key => $name) {
            $Graha = Graha::getInstance($key, $options);
//            $result[$key] = $Graha->grahaDrishti;
            $tmp = [];

            foreach($Graha->grahaDrishti as $planet => $value)
            {
               if($value == 1)
               {
                   $tmp[] = $planet-1;
     ...
     * @return array
     */
    public function isExaltationByGraha($options = null)
    {
        $isExalt = [];
        foreach (Graha::$graha as $key => $name) {
            $Graha = Graha::getInstance($key, $options);
            $isExalt[$key] = $Graha->grahaUcha['rashi'];
        }
    ...
    /**
     * Get Debilitation by grahas.
     *
     * @param null|array $options Options to set (optional)
     * @return array
     */
    public function isDebilitationByGraha($options = null)
    {
        $result = [];
        foreach (Graha::$graha as $key => $name) {
            $Graha = Graha::getInstance($key, $options);
            if(!empty($Graha->grahaNeecha['degree']))
            {
                $result[$key] = $Graha->grahaNeecha['rashi'];
            }
        }
