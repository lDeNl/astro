...
class Data
{
    ...
    const BLOCK_NAKSHATRA = 'nakshatra';
    /**
     * Rising block
     */
    ...
    public function calcNakshatra($graha)
    {
        $AngaDefiner = new AngaDefiner($this);
        
        foreach($graha as $key=>$value) {
            $generateAnga = $AngaDefiner->getNakshatra(false, false, $key);
            foreach ($generateAnga as $anga => $data) {
                $this->data[self::BLOCK_NAKSHATRA][$key][$anga] = $data;
            }
        }
        return $this;
    }
